...
export default agentTest('Claude Code runs the mocked release note workflow end to end', async (t) => {
  t.prompt([
  ...
  expect(result).toHaveCalledToolTimes('get_feature_spec', 1);
  expect(result).toHaveCalledToolTimes('get_bug_report', 1);
  expect(result).toHaveCalledToolTimes('submit_release_note', 1);
